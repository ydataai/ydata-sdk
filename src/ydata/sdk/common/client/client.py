...
class Client(metaclass=SingletonClient):
    ...
    def __init__(self, credentials: Optional[Union[str, dict]] = None, project: Optional[Project] = None, set_as_global: bool = False):
        ...
        self._scheme = 'https'
        # For now, credentials is assumed to be only the bearer token
    ...
    def post(self, endpoint: str, data: Optional[dict] = None, json: Optional[dict] = None, files: Optional[dict] = None, raise_for_status=True) -> Response:
    ...
    def get_static_file(self, endpoint: str, raise_for_status=True) -> Response:
        url_data = self.__build_url(endpoint)
